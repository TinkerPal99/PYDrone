...
class PiControl:
    def __init__(self, address, path):
        self.__gyro_out = [int(0)]
        self.destination_address = address
        self.__cam = PiCamera()
        self.path_for_savings = path

    # ----------------------------cam----------------------------------------------------------------
    def setSettings_of_Cam(self, width, height, framerate):
    ...
    def call_address(self, address_appendix):
        urllib2.urlopen(self.destination_adress + address_appendix)
    ...
    def gyro_read(self):
        ...
        __gy_x_skal = __gyroskop_xout / 100
        __gy_y_skal = __gyroskop_yout / 100
        __gy_z_skal = __gyroskop_zout / 100
        time.sleep(0.1)

        self.__gyro_out.insert(0, __gy_x_skal)
        self.__gyro_out.insert(1, __gy_y_skal)
        self.__gyro_out.insert(2, __gy_z_skal)
        # print (gy_x_skal)
        # print (gy_y_skal)
        # print (gy_z_skal)
    ...
    def gyro_CallOnRead(self, __gyro_out):
        if self.__gyro_out[1] >= 40:
            print("forward")
            self.call_address("RPS1.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS1.php")
        elif self.__gyro_out[1] <= -40:
            print("backward")
            self.call_address("RPS5.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS5.php")
        elif self.__gyro_out[0] >= 50:
            print("right")
            self.call_address("RPS5.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS4.php")
        elif self.__gyro_out[0] <= -50:
            print("left")
            self.call_address("RPS3.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS3.php")
        elif self.__gyro_out[2] >= 50:
            print("down")
            self.call_address("RPS2.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS2.php")
        elif self.__gyro_out[2] <= -50:
            print("up")
            self.call_address("RPS2.php")
            urllib2.urlopen("http://192.168.8.200/main/RPS2.php")
